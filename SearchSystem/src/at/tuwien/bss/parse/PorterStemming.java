...
/**
 * @author Fotis Lazarinis (Glasgow Information Retrieval Group)
 * 
 * @see {@link http://ir.dcs.gla.ac.uk/resources/linguistic_utils/porter.java}
 */
public class PorterStemming {
	
	/**
	 *  takes a String as input and returns the stem of this String again as a String
	 *  
	 * @param token
	 * 			token to be stemmed
	 * @return stemmed token
	 */
	public String stripAffixes( String token ) {

		token = token.toLowerCase();
		token = Clean(token);

		if (( token != "" ) && (token.length() > 2)) {
			token = stripPrefixes(token);

			if (token != "" ) 
				token = stripSuffixes(token);
